...
#### Functions ==============================================================================================
source("/home/nelazzabi/benchmark/co-expression/mathys/functions/filterSCmatrix-simple.R")
...
for (file in file_paths) {
    message(paste0("Storing genes for: ", basename(file))) 
...
process_celltype_files <- function(cell_type, meta_data, dir) {
  ...
  files <- list.files(path = dir, pattern = paste0(cell_type, "_ling"), full.names = TRUE)
  ...
  for (file in files) {
    ...
    for (i in 1:length(patient_ids)) {
        ...
        donor_meta <- data$meta[ data$meta$DONOR == patient_id , ] %>%
...
# Parallelize the processing of each cell type
num_cores <- 6
message(paste0("Using ", num_cores, " cores for parallel processing."))

results <- mclapply(cell_types, function(cell_type) {
  tryCatch({
