...
file_path = os.path.join(input_dir, "velmeshev.h5ad")
...
# Apply mapping
adata.obs["Cell_type"] = adata.obs["cluster"].map(celltype_map)
...
wrangled_path = os.path.join(output_dir, "Velmeshev-2019.h5ad")
...
for subset_name, dataset, outdir in [
    ...
    for cell_type in valid_cell_types:
        ...
        if adata_sub.n_obs == 0:
            continue
        outfile = os.path.join(outdir, f"{cell_type}_Velmeshev-2019.h5ad")
...
# Save to CSV
summary_out = os.path.join(output_dir, "Velmeshev-2019_cell_counts_all2.csv")
