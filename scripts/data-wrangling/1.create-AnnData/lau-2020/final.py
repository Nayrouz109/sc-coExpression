...
file_path = os.path.join(input_dir, "lau.h5ad")
...
celltype_map = {
    ...
    "Astro": "Ast",
    "Mic": "Mic",
    "Opc": "Opc"   # just in case Opc exists
}
adata.obs["Cell_type"] = adata.obs["Cell_type"].map(celltype_map).astype("category")

# Remap disease values
disease_map = {"AD": "AD", "Control": "CTL"}
adata.obs["disease"] = adata.obs["disease"].map(disease_map).astype("category")

# Rename CONDITION → diagnosis
adata.obs = adata.obs.rename(columns={"CONDITION": "diagnosis"})
...
wrangled_path = os.path.join(output_dir, "Lau-2020.h5ad")
...
for subset_name, dataset, outdir in [
    ...
    for cell_type in valid_cell_types:
        ...
        if adata_sub.n_obs == 0:
            continue
        outfile = os.path.join(outdir, f"{cell_type}_Lau-2020.h5ad")
...
# Save to CSV
summary_out = os.path.join(output_dir, "Lau-2020_cell_counts_all2.csv")
