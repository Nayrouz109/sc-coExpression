...
# read in data ==================================
hoffman_data = '/cosmos/data/downloaded-data/AD-meta/Hoffman-2023/data/expr/PsychAD_r0_Dec_28_2022.h5ad'
print("reading Hoffman data\n") 
data = ad.read_h5ad(hoffman_data) 
data.obs = data.obs.rename(columns={"SubID": "patientID"})

print("saving Hoffman patient metadata\n") 
columns_of_interest = [
    'patientID', 'Channel', 'Brain_bank', 
    'round_num', 'poolID', 'Sex', 
    'Age', 'Dx_AD', 'Ethnicity', 
    'pH', 'PMI'
]
patient_metadata = data.obs[columns_of_interest]
patient_metadata = patient_metadata.loc[:, ~patient_metadata.columns.duplicated()]
patient_metadata.reset_index(drop=True, inplace=True) #reset the index

#data.obs[data.obs['Dx_AD'] == 'Control']['patientID'].unique()
#data.obs[data.obs['Dx_AD'] == 'AD']['patientID'].unique()
patient_metadata.to_csv("/cosmos/data/downloaded-data/AD-meta/Hoffman-2023/data/expr/patient_metadata", index=False)

# cell type mapping  ====================================================
data.obs["subclass"].unique().tolist()
cell_types = {
    "Exc": data.obs['class'].isin(['EN']),
    "Inh": data.obs['class'].isin(['IN']),
    "Ast": data.obs['subclass'].isin(['Astro']),
    "Oli": data.obs['subclass'].isin(['Oligo']),
    "Opc": data.obs['subclass'].isin(['OPC']),
    "Mic": data.obs['subclass'].isin(['Micro_PVM']),
}
...
for cell_type, condition in cell_types.items():
    ...
    file_name = f"{cell_type}_Hoffman-2023.h5ad"  
