...
num_cores <- 15  # Use all but one core
...
# Step 3: Save results for glutamatergic cell type in 3 batches
glutamatergic_results <- lapply(all_results, function(res) res[[1]])  # Only "glutamatergic" cell type

# Calculate batch sizes
batch_size <- ceiling(length(glutamatergic_results) / 3)

# Create output directory for glutamatergic cell type
output_dir_glutamatergic <- "/space/scratch/nairuz-rewiring-normal/data/processed/ling/glutamatergic"
dir.create(output_dir_glutamatergic, recursive = TRUE, showWarnings = FALSE)

# Process and save results in 3 chunks
for (batch_num in 1:3) {
  start_idx <- (batch_num - 1) * batch_size + 1
  end_idx <- min(batch_num * batch_size, length(glutamatergic_results))

  # Combine meta and expr for the current batch
  meta_combined <- do.call(rbind, lapply(glutamatergic_results[start_idx:end_idx], function(res) res$meta))
  expr_combined <- do.call(cbind, lapply(glutamatergic_results[start_idx:end_idx], function(res) res$expr))
  
  # Save the batch
  output_file <- paste0(output_dir_glutamatergic, "/glutamatergic_batch_", batch_num, ".rds")
  saveRDS(list(meta = meta_combined, expr = expr_combined), output_file)
  message(paste("Saved batch", batch_num, "to", output_file))
}

#### run the script ============================================================
#chmod +x glutamatergic-chunks-parallele.r
#Rscript glutamatergic-chunks-parallele.r "BA46_(05-07-2019|05-21-2021|05-28-2021|07-10-2019|08-09-2019|08-27-2019|09-02-2021|09-03-2021|09-18-2019|09-22-2021|10-16-2019)_rxn[0-9]+\\.h5"
