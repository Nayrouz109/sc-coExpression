...
output_dir = Path("/space/scratch/nairuz-rewiring/data/2.prcsd-slcGenes-cpm/intrsct-lessStringent-diag")
...
for file_path in input_dir.glob("*.h5ad"):
    ...
    #missing_patient_ids = [pid for pid in p_meta_study_patient_ids if pid not in ad_patient_ids]
    ...
    # Integrate diagnosis information
    adata.obs = adata.obs.merge(
        p_meta_study[["patientID", "diagnosis"]], 
        on="patientID", 
        how="left") 
    ...
    try:
        ...
        print(f"Integrated diagnosis into {file_path.stem} and saved to {output_file_path}")
    except Exception as e:
