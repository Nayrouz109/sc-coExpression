...
def compute_fisher_XcellXsubj(lnk_tables_dir, 
    ...
    for cell in cell_types:
        ...
        def run_fisher(curr_dat_T, curr_dat_S):
            return (
                curr_dat_T,
                curr_dat_S,
                fisher_test(
        ...
        for curr_dat_T in lnks_T.keys():
            ...
            for curr_dat_S in datasets_S:
                tasks.append((curr_dat_T, curr_dat_S))

        # Run in parallel
        parallel_results = Parallel(n_jobs=n_jobs)(
            delayed(run_fisher)(curr_dat_T, curr_dat_S) for curr_dat_T, curr_dat_S in tasks
        )

        # Rebuild nested dict structure
        res = {}
        for curr_dat_T, curr_dat_S, fisher_res in parallel_results:
            if curr_dat_T not in res:
                res[curr_dat_T] = {}
            res[curr_dat_T][curr_dat_S] = fisher_res
