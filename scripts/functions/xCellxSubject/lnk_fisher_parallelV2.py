...
def compute_fisher_XcellXsubj(lnk_tables_dir, output_dir, cell_types=None, condition=None, n_jobs=-1):
    """
    ...
    # Helper to run Fisher for one comparison
    def run_fisher(curr_dat_T, curr_dat_S, lnks_T, lnks_F):
        print(f"    Comparing: {curr_dat_S} (predicted) vs {curr_dat_T} (true set)")
        return curr_dat_S, fisher_test(
    ...
    if cell_types is None or not cell_types:
        ...
        for fpath in all_files:
            ...
            tbl = pd.read_parquet(fpath) if fpath.endswith(".parquet") else pd.read_csv(fpath)
        ...
        for curr_dat_T in lnks_T.keys():
            ...
            fisher_results = Parallel(n_jobs=n_jobs)(
                delayed(run_fisher)(curr_dat_T, curr_dat_S, lnks_T, lnks_F)
                for curr_dat_S in datasets_S
            )
            res[curr_dat_T] = {curr_dat_S: fisher_res for curr_dat_S, fisher_res in fisher_results}
