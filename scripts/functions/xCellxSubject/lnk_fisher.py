...
def process_lnktbles(input_dir, output_dir, cell_types, condition):
    """
    Reads .h5ad files in input_dir that match given cell_types,
    extracts lower triangle of coexpression matrices,
    and saves as parquet tables.
    """
    os.makedirs(output_dir, exist_ok=True)

    # List all h5ad files in input_dir
    #exprfiles = [os.path.join(input_dir, f) for f in os.listdir(input_dir) if f.endswith(".h5ad")]
    exprfiles = [
    ...
    for cell in cell_types:
        ...
        coexmats = [f for f in exprfiles if re.match(cell, os.path.basename(f))] # match from start

        if not coexmats:
            print(f"No files found for {cell}, skipping...")
            continue

        for fpath in coexmats:
            ...
            # --- Add canonical gene ordering here ---
            #canonical_genes = sorted(adata.var_names)
            #adata = adata[:, canonical_genes]  # reorder columns to canonical order

            # Assume coexpression matrix is stored in .X
            ...
            # Thresholding like in your R code
            ...
            #out_file = os.path.join(output_dir, f"{dataset_name}_xcellxSubject_reprod.parquet")
...
def compute_fisher_XcellXsubj(lnk_tables_dir, 
                              output_dir, 
                              cell_types=None, 
                              condition=None
                              #n_jobs=4
    ...
    #all_files = [os.path.join(lnk_tables_dir, f) 
    #             for f in os.listdir(lnk_tables_dir) if f.endswith("_xcellxSubject_reprod.parquet") or f.endswith("_xcellxSubject_reprod.csv")]
    
    all_files = [
        os.path.join(lnk_tables_dir, f) 
        for f in os.listdir(lnk_tables_dir) 
        if (
            (f.endswith(f"_xcellxSubject_reprod_{condition}.parquet") or 
            f.endswith(f"_xcellxSubject_reprod_{condition}.csv"))
        )  
    ]
    ...
    # Detect cell types automatically if not provided
    if cell_types is None:
        detected = set()
        for f in all_files:
            m = re.match(r"(\w+)_", os.path.basename(f))
            if m:
                detected.add(m.group(1))
        cell_types = sorted(list(detected))
    ...
    for cell in cell_types:
        ...
        #print("  Running Fisher's tests in parallel...")
        ...
        #    def compute_for_dataset(curr_dat_S):
        #       return fisher_test(
        #           predicted=lnks_T[curr_dat_S],
        #           not_predicted=lnks_F[curr_dat_S],
        #           true_set=lnks_T[curr_dat_T],
        ...
        #   res[curr_dat_T] = Parallel(n_jobs=n_jobs)(
        #       delayed(compute_for_dataset)(d) for d in datasets_S
        #   )
        #   res[curr_dat_T] = dict(zip(datasets_S, res[curr_dat_T]))
        
        #results[cell] = res
    
    # Save results
    # Add condition label to output filenames if provided
    suffix = f"_{condition}" if condition else ""
