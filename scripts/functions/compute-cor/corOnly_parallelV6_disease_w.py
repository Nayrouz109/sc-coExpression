...
def process_patient(patient_id, adata, min_cells_threshold, correlation_type, replace_nans):
    ...
    Adds weight to patient matrix based on the total number of single cells in the patient.
    
    Returns:
        tuple -- Patient ID and a dictionary with processed edge lists, rank-normalized matrix, and weight.
    """
    patient_data = adata[adata.obs['patientID'] == patient_id]
    num_cells = patient_data.X.shape[0]

    if num_cells < min_cells_threshold:
        print(f"Skipping patient {patient_id} due to insufficient number of cells.")
        return patient_id, None
    
    # Compute correlation matrix
    if correlation_type == 'pearson':
        corr_matrix = sparse_corr(scipy.sparse.csr_matrix(patient_data.X))
    elif correlation_type == 'spearman':
        corr_matrix, _ = spearmanr(patient_data.X, axis=0, nan_policy='omit')
    else:
    ...
    return patient_id, {
        ...
        'weight': num_cells  # Weight based on the total number of single cells in this patient
    }
...
def compute_gene_correlation(adata, 
    ...
    gene_names = None
    patient_count = 0
    total_cells_passing_threshold = 0  # Track single cells from valid patients
    ...
    for patient_id in patient_ids:
        ...
        if result is not None and result[1] is not None:
            ...
            patient_weight = result[1]["weight"]  # Extract weight (total single cells in the patient)
            total_cells_passing_threshold += patient_weight  # Accumulate for summary

            if gene_names is None:
                gene_names = result[1]["gene_names"]

            # Aggregate patient matrix
            if aggregated_matrix is None:
                aggregated_matrix = patient_matrix * patient_weight  # Weight contribution
            else:
    ...
    # Normalize the aggregated matrix by the total number of contributing patients (weighted mean)
    ...
    summary_df = pd.DataFrame({
        ...
        "total_single_cells": [total_cells_passing_threshold],  # Only valid patient cells counted
        "total_patients": patient_count,
        "aggregate_weight": [patient_count]  # Weight for the aggregated matrix
    })
